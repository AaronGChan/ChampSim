...
def champsim_config_reader(champsim_config_file_path):
    res = {}
    with open(champsim_config_file_path, "r") as json_file:
    ...
    res["L3_config"] = l3_capacity + f",64,{l3_ways},8,8,15,64,1"
    ...
    res["L2_config"] = l2_capacity + f",64,{l2_ways},8,8,15,64,1"
    ...
    l1d_ways = champsim_config["L1D"]["sets"]
    res["dcache_config"] = l1d_capacity + f",64,{l1d_ways},1,8,5,64,1"

    res["icache_buffer_sizes"] = str(champsim_config["L1D"]["mshr_size"]) + ",64,64,32"
    l1i_capacity = str(
        champsim_config["block_size"]
    ...
    if champsim_config["ooo_cpu"][0]["branch_predictor"] == "basic_btb":
        res["BTB_config"] = "5120,4,2,1,1,3"
    else:
        res["BTB_config"] = "10240,4,2,1,1,3"
...
def xml_reader(data, champsim_terminal_output):
    ...
    res["L30_read_accesses"] = str(
        data["roi"]["LLC"]["RFO"]["hit"][0] + data["roi"]["LLC"]["RFO"]["miss"][0]
    )
    res["L30_read_misses"] = str(data["roi"]["LLC"]["RFO"]["miss"][0])
    ...
    res["L20_read_accesses"] = str(
        data["roi"]["cpu0_L2C"]["RFO"]["hit"][0] + data["roi"]["cpu0_L2C"]["RFO"]["miss"][0]
    )
    res["L20_read_misses"] = str(data["roi"]["cpu0_L2C"]["RFO"]["miss"][0])
    ...
    res["dcache_read_accesses"] = str(
        data["roi"]["cpu0_L1D"]["RFO"]["hit"][0] + data["roi"]["cpu0_L1D"]["RFO"]["miss"][0]
    )
    res["dcache_read_misses"] = str(data["roi"]["cpu0_L1D"]["RFO"]["miss"][0])
    ...
    res["icache_read_accesses"] = str(
        data["roi"]["cpu0_L1I"]["RFO"]["hit"][0] + data["roi"]["cpu0_L1I"]["RFO"]["miss"][0]
    )
    res["icache_read_misses"] = str(data["roi"]["cpu0_L1I"]["RFO"]["miss"][0])
    ...
    for each_idx in range(len(all_lines)):
        if keywords[0] in all_lines[each_idx]:
            ...
            result = float(result.group(1).split("%")[0])
            br_accuracy = result
        ...
        if keywords[3] in all_lines[each_idx]:
            ...
            if result.group(1) != "Channel0":
                ...
                result = 0
            WQ_ROW_BUFFER_HIT = result
            my_string = all_lines[each_idx + 1]
            result = re.search("ROW_BUFFER_MISS:(.*)", my_string)
            result = int(result.group(1))
            WQ_ROW_BUFFER_MISS = result
    ...
    try:
        res["branch_instructions"] = int(res["branch_mispredictions"]) / (
...
def set_xml(res, xml_path, output_file_path):
    ...
    #     breakpoint()
    #     if stat.get('name') == "total_instructions" or stat.get('name') == "int_instructions":
    #         stat.set('value', res["total_instructions"])
    #     elif stat.get('name') == "fp_instructions":
    #         stat.set('value',res["fp_instructions"] )
    #     elif stat.get('name') == "branch_instructions":
    ...
    #     elif stat.get('name') == "branch_mispredictions":
    #         stat.set('value',res["branch_mispredictions"])
    #     elif stat.get('name') == "committed_instructions" or "committed_int_instructions":
    #         stat.set('value',res["committed_instructions"])
    #     elif stat.get('name') == "committed_fp_instructions":
    #         stat.set('value',res["committed_fp_instructions"])
    for param in root.iter("param"):
        # Change the value to your desired number of cores
        if (
            ...
            )  # Change the value to your desired clock rate
