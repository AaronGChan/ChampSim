...
frequency_mapper = {0: 1000, 1: 1250, 2: 1500, 3:1750, 4:2000, 5:2250, 6:2500, 7:2750, 8:3000, 9:3250, 10:3500, 11:3750, 12:4000, 13:4250, 14:4500}
...
rob_size_mapper = {0:16, 1:32, 2:48, 3:64, 4:80, 5:96, 6:112, 7:128, 8:144, 9:160, 10:176, 11:192, 12:208, 13:224, 14:240, 15:256, 16:320, 17: 384, 18: 448, 19:512, 20: 576}
...
scheduler_size_mapper = {0:2, 1:4, 2:8, 3:16, 4:32, 5:64, 6:128, 7:144, 8:160, 9:176, 10:192, 11:208, 12:256}
branch_predictor_mapper = {0: "bimodal", 1: "gshare", 2: "hashed_perceptron", 3:"perceptron"}
...
l1d_prefetcher_mapper = {0:"ip_stride", 1:"next_line", 2:"no", 3:"spp_dev", 4:"va_ampm_lite"}

def decode(act_encoded):
    act_decoded = {}
    if isinstance(act_encoded, dict):
        act_decoded["Frequency"] = frequency_mapper[act_encoded["Frequency"]]
        act_decoded["iFetchBufferSize"] = ifetch_buffer_size_mapper[act_encoded["iFetchBufferSize"]]
        act_decoded["DecodeBufferSize"] = decode_buffer_size_mapper[act_encoded["DecodeBufferSize"]]
        act_decoded["DispatchBufferSize"] = dispatch_buffer_size_mapper[act_encoded["DispatchBufferSize"]]
        ...
        act_decoded["DispatchWidth"] = dispatch_width_mapper[act_encoded["DispatchWidth"]]
        ...
        act_decoded["SchedulerSize"] = scheduler_size_mapper[act_encoded["SchedulerSize"]]
        act_decoded["BranchPredictor"] = branch_predictor_mapper[act_encoded["BranchPredictor"]]
        ...
        act_decoded["L1IPrefetcher"] = l1i_prefetcher_mapper[act_encoded["L1IPrefetcher"]]
        ...
        act_decoded["L1DPrefetcher"] = l1d_prefetcher_mapper[act_encoded["L1DPrefetcher"]]
    return act_decoded
...
def select_config():
    ...
    while not act_encoded or repr(list(act_encoded.values())) in data_store.keys():
        act_encoded["Frequency"] = random.randint(0, len(frequency_mapper.keys()) - 1)
        act_encoded["iFetchBufferSize"] = random.randint(0, len(ifetch_buffer_size_mapper.keys()) - 1)
        act_encoded["DecodeBufferSize"] = random.randint(0, len(decode_buffer_size_mapper.keys()) - 1)
        act_encoded["DispatchBufferSize"] = random.randint(0, len(dispatch_buffer_size_mapper.keys()) - 1)
        ...
        act_encoded["FetchWidth"] = random.randint(0, len(fetch_width_mapper.keys()) - 1)
        act_encoded["DecodeWidth"] = random.randint(0, len(decode_width_mapper.keys()) - 1)
        act_encoded["DispatchWidth"] = random.randint(0, len(dispatch_width_mapper.keys()) - 1)
        act_encoded["ExecuteWidth"] = random.randint(0, len(execute_width_mapper.keys()) - 1)
        ...
        act_encoded["RetireWidth"] = random.randint(0, len(retire_width_mapper.keys()) - 1)
        act_encoded["SchedulerSize"] = random.randint(0, len(scheduler_size_mapper.keys()) - 1)
        act_encoded["BranchPredictor"] = random.randint(0, len(branch_predictor_mapper.keys()) - 1)
        act_encoded["BTB"] = random.randint(0, len(btb_mapper.keys()) - 1)
        act_encoded["DIBWindowSize"] = random.randint(0, len(window_size_mapper.keys()) - 1)
        ...
        act_encoded["L1IRQSize"] = random.randint(0, len(l1i_rq_size_mapper.keys()) - 1)
        act_encoded["L1IWQSize"] = random.randint(0, len(l1i_wq_size_mapper.keys()) - 1)
        act_encoded["L1IPQSize"] = random.randint(0, len(l1i_pq_size_mapper.keys()) - 1)
        act_encoded["L1IMSHRSize"] = random.randint(0, len(l1i_mshr_size_mapper.keys()) - 1)
        act_encoded["L1IPrefetcher"] = random.randint(0, len(l1i_prefetcher_mapper.keys()) - 1)
        ...
        act_encoded["L1DRQSize"] = random.randint(0, len(l1d_rq_size_mapper.keys()) - 1)
        act_encoded["L1DWQSize"] = random.randint(0, len(l1d_wq_size_mapper.keys()) - 1)
        act_encoded["L1DPQSize"] = random.randint(0, len(l1d_pq_size_mapper.keys()) - 1)
        act_encoded["L1DMSHRSize"] = random.randint(0, len(l1d_mshr_size_mapper.keys()) - 1)
        act_encoded["L1DPrefetcher"] = random.randint(0, len(l1d_prefetcher_mapper.keys()) - 1)
    act_decoded = decode(act_encoded)
    write_to_json(act_decoded)
    return act_encoded

def write_to_json(action):
    champsim_ctrl_file = "/Users/aaronchan/Documents/ChampSim/champsim_config.json"
...
def run_program(iter, action_dict):
    ...
    process = subprocess.Popen(["./config.sh", "champsim_config.json"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if err.decode() == "":
        ...
        print(err.decode())
        sys.exit()
    process = subprocess.Popen(["make", "-j2"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = process.communicate()
